...
from datasets.utils import build_data_loader, FewShotData
import clip
from utils import *
...
def main_datvilc(cfg):
    ...
    attributes_weights = clip_attributes(dataset.classnames, dataset.attributes, clip_model)
    ...
    candidate_classes = extract_candidates(cfg, network, test_features, dataset.classnames)
    ...
    for i in range(samples_num):
        ...
        if label_key1 in attributes_discriminative.keys():
            ...
            try:
                ...
                continue
        #else:
        #    raise KeyError('Context-aware descriptions for {}:{} not found. Please update the descriptions with the information of the new classes.'.format(label_key1[0], label_key1[1]))
...
def main_datvil(cfg):    
    ...
    few_shot_data_class = FewShotData(dataset.train_x, dataset.attributes, attributes_weights, dataset.classnames, clip_model,  input_size= 224, transform=train_tranform, is_train=True)
